...
- **گیتِ پرداخت (پیاده‌شد ✅ — اعتبارِ جلسه):** اگر `Therapist.productId` ست شده باشد، رزرو نیازمندِ «اعتبارِ جلسه» است. با پرداختِ سفارشِ حاویِ آن محصول (وضعیتِ `PROCESSING`)، `ClinicAccessService.grantSessionCredits` به‌تعدادِ خریداری‌شده اعتبار اضافه می‌کند؛ هر رزرو یک واحد کم می‌کند و لغوِ نوبت آن را برمی‌گرداند. `TherapistDetailResponse` شاملِ `sessionCredits`/`requiresPurchase`/`productSlug` است و صفحه‌ی جزئیاتِ درمانگر در کلاینت وضعیتِ خرید را نشان می‌دهد و به‌طورِ مستقیم به صفحه‌ی محصول لینک می‌دهد. درمانگرهایی بدونِ `productId` رایگان و بی‌نیاز از خرید می‌مانند. migration `024_add_session_credits.sql`.

### پنلِ ادمینِ کلاینت برای آموزشگاه و مشاوره — پیاده‌سازی‌شده ✅
دو تبِ جدید («آموزشگاه» و «مشاوره») به پنلِ مدیریتِ فروشگاه (`AdminPanelScreen`) اضافه شده‌اند:
- **`:feature:admin:academy`**: ساختِ دوره (عنوان/اسلاگ/قیمت/شناسه‌ی محصول)، فهرستِ دوره‌ها با بازکردنِ هرکدام برای دیدنِ بخش‌ها/درس‌ها، افزودنِ بخش و افزودنِ درس (لینکِ ویدیو + مدت + پیش‌نمایشِ رایگان)، حذفِ دوره. سرور یک endpointِ جدید هم گرفت: `GET /api/admin/courses/{id}` (جزئیاتِ کاملِ دوره برای ادمین، بدونِ گیتِ ثبت‌نام روی لینکِ ویدیو).
- **`:feature:admin:clinic`**: دو زیرتب — «درمانگرها» (ساخت/حذفِ درمانگر + افزودنِ بازه‌ی زمانی با ورودیِ ISO datetime) و «نوبت‌ها» (فهرستِ همه‌ی نوبت‌ها، تأیید + ثبتِ لینکِ اتاقِ تماس، علامت‌زدنِ برگزارشده). سرور دو endpointِ جدید گرفت: `GET /api/admin/therapists/{id}/slots` و `GET /api/admin/therapists/appointments`.
- ورودی‌های فرم (تاریخ/زمانِ بازه) ساده و متنی هستند (فرمتِ ISO-8601 با آفست، مثلِ `2026-07-10T14:00:00+03:30`)؛ افزودنِ date-picker گرافیکی کارِ بعدی است اگر خواستی.

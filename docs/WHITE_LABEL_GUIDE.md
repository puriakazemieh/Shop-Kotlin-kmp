...
- **کلاینت** (`:feature:academy`): صفحاتِ «دوره‌ها»، «دوره‌های من»، «جزئیات دوره» (ثبت‌نام + نوارِ پیشرفت) و «یادگیری» (پخشِ ویدیو با بازاستفاده از `VideoPlayer` ماژولِ details + علامت‌زدنِ تکمیل + درصد پیشرفت). ورودی از «دوره‌های من» در صفحه‌ی «مشخصات».
- **نکته‌ی محصولِ واقعی:** `enroll` فعلاً مستقیم اجازه می‌دهد؛ در محصولِ نهایی باید پس از پرداختِ موفق (یا رویدادِ تکمیلِ سفارش) فراخوانی شود. کافی است `Course.productId` را به محصولِ فروشگاه لینک کنی و در `OrderService` هنگامِ تکمیلِ سفارش، `enroll` را صدا بزنی.
- **داده‌ی نمونه:** از پنل ادمین (`POST /api/admin/courses` → افزودن section و lesson) یک دوره بساز تا در اپ ببینی.

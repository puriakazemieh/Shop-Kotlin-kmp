> **این سند تکمیل شده است ✅** — همه‌ی فازهای A تا E انجام شده‌اند (فقط فازِ F یعنی build/QA
> روی دستگاه شما باقی است). برای نقشه‌ی راهِ **پلتفرمِ چندعمودی/چند‌برند** (آموزشگاه، مشاوره،
> تست روان‌شناسی، ساعت، عطر، مقایسه‌ی محصول و...) به سندِ جدید مراجعه کنید:
> **[`MULTI_VERTICAL_PLATFORM_PLAN.md`](./MULTI_VERTICAL_PLATFORM_PLAN.md)**
...
> - **فاز A** (صفحه‌ی محصول: مفید بود، موجود شد خبرم کن، فیت/ارسال) — ✅ انجام شد (سرور + کلاینت)
> - **فاز B** (صفحه‌ی جستجوی مستقل + جستجوهای اخیر/پرطرفدار) — ✅ انجام شد
> - **فاز C** (چت پشتیبانی: دامنه‌ی تیکت سرور + رابطِ چت کلاینت) — ✅ انجام شد
> - **فاز D** (تاریخچه‌ی زمان‌دارِ رهگیری سفارش) — ✅ انجام شد (سرور + کلاینت)
> - **فاز E** (فیلترِ قیمت/موجودی، برندِ auth، داشبورد ادمین، ممیزی) — ✅ انجام شد (E2/E3 از قبل بودند)
> - **فاز F** (بیلد + QA بصری روی دستگاهِ شما) — ⬜ در انتظارِ اجرای شما
>
> نکته: محیطِ ابری قادر به build نیست؛ صحتِ کامپایل و تأییدِ بصری باید روی دستگاهِ شما انجام شود (بخش ۶).
...
- [x] **D2 کلاینت — بایندِ تایم‌لاینِ واقعی**
...
- [x] **E2 — برندینگِ صفحاتِ auth** — ✅ از قبل انجام شده بود
  - هر چهار صفحه (`LoginScreen`, `RegisterScreen`, `ForgotPasswordScreen`, `ResetPasswordScreen`)
    از `component/AuthBrandHeader` استفاده می‌کنند (لوگوباجِ «ک» + عنوانِ کارمیلا).

- [x] **E3 — داشبوردِ آماریِ ادمین** — ✅ از قبل انجام شده بود
  - `AdminPanelScreen.kt` → `AdminDashboard(stats)` کارت‌های درآمد/سفارش/محصول/مشتری + «نمودار فروش هفتگی»
    را از `GetAdminStatsUseCase` (روی `GET /api/admin/stats`) نشان می‌دهد.

- [x] **E4 — ممیزیِ نهاییِ اجزاء مشترک** — ✅ بازبینی شد
  - هدر/نوارِ ناوبریِ پایین/Toast(`ContentWithMessageBar`)/Modal با اسپک منطبق‌اند (کامپوننت‌های کارمیلا از فاز ۱).

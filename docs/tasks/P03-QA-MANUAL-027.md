...
# P03-QA-MANUAL-027 — QA انسانی جایگزین Manifest پس از اتصال runtime
...
با build مشخص فعلی و WordPress staging، toggle، لینک مستقیم، restart و خطای manifest را واقعاً تست کن. این شاهد جای اتکا به نتیجه ناقص QA020 را می‌گیرد. Spring fixture فقط test contract است؛ QA سرور واقعی در P15 انجام می‌شود.
...
- `docs/evidence/P03-QA-MANUAL-027/**`
- `docs/tasks/P03-QA-MANUAL-027.md`
- `docs/tasks.md`
- `docs/MASTER_IMPLEMENTATION_CHECKLIST_FA.md`

- مستندات قرارداد مرتبط و `docs/evidence/P03-QA-MANUAL-027/**`؛ row/card/checklist همین Task.
...
کجا: wp-admin تنظیم قابلیت و app staging. چگونه: روشن/خاموش، refresh بدون rebuild، لینک مستقیم و قطع درخواست manifest. موفقیت: UI/route/network/API همسو؛ tester و build و نتیجه هر گام ثبت؛ تا تأیید انسان AWAITING_MANUAL_QA.

## Automated tests با command و expected result

پیش از QA، build fingerprint و گزارش تست خودکار dependencyها را ثبت کن؛ تست دستی را با ابزار خودکار جایگزین نکن.
...
- موفقیت: کجا: wp-admin تنظیم قابلیت و app staging. چگونه: روشن/خاموش، refresh بدون rebuild، لینک مستقیم و قطع درخواست manifest. موفقیت: UI/route/network/API همسو؛ tester و build و نتیجه هر گام ثبت؛ تا تأیید انسان AWAITING_MANUAL_QA.
...
- مسیر: `docs/evidence/P03-QA-MANUAL-027/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

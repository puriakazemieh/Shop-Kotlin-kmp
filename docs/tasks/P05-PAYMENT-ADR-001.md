# P05-PAYMENT-ADR-001 — قرارداد پرداخت مشترک دو میزبان و ماتریس قابلیت provider
...
- Status: TODO
- Phase/Area/Type: P05 / PAYMENT / ADR
...
قرارداد create/verify/query/reverse/refund/deliver/settlement در Shared Core سمت سرور تعیین شود؛ Theme-only و Plugin-only همان write path را دارند و WooCommerce مرجع سفارش است.

## خروجی مورد انتظار
جدول owner و API برای هر عملیات؛ مجوز دامنه و capability provider جدا؛ merchant secret در UI و کلاینت صفر.
...
3. قرارداد/مستند این کارت را با موارد زیر تطبیق و تصمیم‌های باز را ownerدار ثبت کن: قرارداد create/verify/query/reverse/refund/deliver/settlement در Shared Core سمت سرور تعیین شود؛ Theme-only و Plugin-only همان write path را دارند و WooCommerce مرجع سفارش است.
4. معیار اختصاصی را با Evidence قابل بازتولید بررسی کن: جدول owner و API برای هر عملیات؛ مجوز دامنه و capability provider جدا؛ merchant secret در UI و کلاینت صفر.
...
- معیار اختصاصی: جدول owner و API برای هر عملیات؛ مجوز دامنه و capability provider جدا؛ merchant secret در UI و کلاینت صفر.
...
- معیار موفقیت: جدول owner و API برای هر عملیات؛ مجوز دامنه و capability provider جدا؛ merchant secret در UI و کلاینت صفر.
...
## Evidence
- مسیر: docs/evidence/P05-PAYMENT-ADR-001/

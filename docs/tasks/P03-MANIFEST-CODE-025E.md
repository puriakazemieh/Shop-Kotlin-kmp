...
# P03-MANIFEST-CODE-025E — اعمال guard عملیاتی تست روان‌شناسی
...
- Depends on: P03-MANIFEST-CODE-025D
- Blocks: P03-QA-AUTO-026
...
فقط مسیرهای repository/use-case/background دامنه تست روان‌شناسی را به وضعیت جاری feature متصل کن؛ قبل از ارسال درخواست guard اجرا شود و خاموشی حین عملیات با policy لغو/اتمام امن مشخص باشد. کنترل سروری همچنان الزامی است.
...
- `core/data/** و core/network/** فقط حوزه psychtest`
- `feature/** فقط مصرف‌کننده همان حوزه`
- `composeApp/** فقط wiring/test`

- مستندات قرارداد مرتبط و `docs/evidence/P03-MANIFEST-CODE-025E/**`؛ row/card/checklist همین Task.
...
با transport شمارنده واقعی در DI، feature خاموش صفر request ایجاد کند؛ روشن مجاز request درست؛ auth/ownership با toggle دور زده نشود.
...
- موفقیت: با transport شمارنده واقعی در DI، feature خاموش صفر request ایجاد کند؛ روشن مجاز request درست؛ auth/ownership با toggle دور زده نشود.
...
- مسیر: `docs/evidence/P03-MANIFEST-CODE-025E/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

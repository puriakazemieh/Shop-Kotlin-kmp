...
# P03-ARCH-CODE-024B — تزریق BuildSpec صریح در bootstrap همه targetها
...
- Depends on: P03-ARCH-CODE-024A
- Blocks: P03-MANIFEST-CODE-025
...
initKoin و entrypointهای Android/Web/JVM/iOS پروفایل و tenant را از BuildSpec بگیرند؛ brand.id نوع backend را تعیین نکند. namespace نشست از tenant+backend+origin ساخته شود.
...
- مستندات قرارداد مرتبط و `docs/evidence/P03-ARCH-CODE-024B/**`؛ row/card/checklist همین Task.
...
نام برند دلخواه روی هر دو profile درست؛ تغییر tenant logout/cache purge؛ entrypointهای فعلی و packageهای منتشرشده حفظ شوند.
...
- موفقیت: نام برند دلخواه روی هر دو profile درست؛ تغییر tenant logout/cache purge؛ entrypointهای فعلی و packageهای منتشرشده حفظ شوند.
...
- مسیر: `docs/evidence/P03-ARCH-CODE-024B/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

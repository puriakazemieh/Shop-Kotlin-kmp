...
# P04-WORDPRESS-DATA-042 — پذیرش داده موجود و جابه‌جایی میزبان بدون حذف
...
- Status: TODO
- Phase/Area/Type: P04 / WORDPRESS / DATA
...
فرآیند adoption metadata/version/option فعلی و انتقال authority میان میزبان‌ها را روی snapshot synthetic پیاده کن؛ داده domain را دوباره کپی نکن. انتقال مجوز صریح و schema compatibility را کنترل کن.
...
- مستندات قرارداد مرتبط و `docs/evidence/P04-WORDPRESS-DATA-042/**`؛ row/card/checklist همین Task.
...
Theme→both→Plugin و برعکس بدون تغییر ID/count/checksum داده؛ توقف migration recovery؛ خاموشی آخرین میزبان داده را حفظ کند.
...
- موفقیت: Theme→both→Plugin و برعکس بدون تغییر ID/count/checksum داده؛ توقف migration recovery؛ خاموشی آخرین میزبان داده را حفظ کند.
...
- مسیر: `docs/evidence/P04-WORDPRESS-DATA-042/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

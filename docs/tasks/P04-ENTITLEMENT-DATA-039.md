...
# P04-ENTITLEMENT-DATA-039 — کاتالوگ قابلیت و SKU با تفکیک حق خرید و toggle
...
- Status: TODO
- Phase/Area/Type: P04 / ENTITLEMENT / DATA
...
- Blocks: P04-WPPLUGIN-CODE-003, P04-ENTITLEMENT-CODE-040, P04-CI-CODE-043, P14-CLINIC-CODE-033
...
schema نسخه‌دار کاتالوگ قابلیت، product kind، artifact inventory و entitlement claims را ثبت و fixture کن؛ پوشش همه featureها و controllerها، dependency closure و شناسه جدا برای معرفی/نوبت/خدمت مشاوره/داده خصوصی/psych و appbuilder.targetها تعیین شود.
...
- مستندات قرارداد مرتبط و `docs/evidence/P04-ENTITLEMENT-DATA-039/**`؛ row/card/checklist همین Task.
...
شناسه‌های قدیمی alias معتبر؛ بسته Base بدون vertical؛ بسته Booking بدون الزام Psych؛ حق builder/platform جدا؛ offline/grace/revoke/upgrade/site transfer صریح.
...
- موفقیت: شناسه‌های قدیمی alias معتبر؛ بسته Base بدون vertical؛ بسته Booking بدون الزام Psych؛ حق builder/platform جدا؛ offline/grace/revoke/upgrade/site transfer صریح.
...
دو ورودی artifactKind=theme/plugin با features و builders مستقل تعریف کن. چهار boolean جدا برای android/ios/desktop/pwa، همه missingها false؛ builder.core از وجود حداقل یک target لازم است و حق سایر targetها را ایجاد نمی‌کند. registry باید mapping feature→module/files/resources/routes/admin/view و dependencies را برای assembler داشته باشد. مجوز محلی امضاشده می‌تواند همراه ZIP باشد؛ سرویس آنلاین اجباری برای کارکرد عادی سایت از این درخواست استنباط نشود.
...
- مسیر: `docs/evidence/P04-ENTITLEMENT-DATA-039/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

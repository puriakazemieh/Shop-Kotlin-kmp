# P12-BUILDER-CODE-008 — تولید config و منابع مشتری از BuildSpec بدون fork
...
- Depends on: P12-BUILDER-SEC-007
- Blocks: P12-BUILDER-CODE-020
- Requirement source: Master checklist row P12-BUILDER-CODE-008 و Source audit بخش BUILDER
...
config/resources از backend، branding، tenant، SKU، entitlement و سقف compiled capability تولید شوند؛ خروجی در هر دو host و ورودی مستقل یکسان باشد.

## خروجی مورد انتظار
BuildSpec یکسان artifact/config یکسان؛ درخواست feature خارج از بسته رد؛ برند backend را تغییر ندهد.
...
- Dependencyها: P12-BUILDER-SEC-007
...
3. تغییر محدود این کارت را در مسیرهای مجاز پیاده یا آزمون کن: config/resources از backend، branding، tenant، SKU، entitlement و سقف compiled capability تولید شوند؛ خروجی در هر دو host و ورودی مستقل یکسان باشد.
4. معیار اختصاصی را با Evidence قابل بازتولید بررسی کن: BuildSpec یکسان artifact/config یکسان؛ درخواست feature خارج از بسته رد؛ برند backend را تغییر ندهد.
...
- معیار اختصاصی: BuildSpec یکسان artifact/config یکسان؛ درخواست feature خارج از بسته رد؛ برند backend را تغییر ندهد.
...
- معیار موفقیت: BuildSpec یکسان artifact/config یکسان؛ درخواست feature خارج از بسته رد؛ برند backend را تغییر ندهد.
...
## Evidence
- مسیر: docs/evidence/P12-BUILDER-CODE-008/

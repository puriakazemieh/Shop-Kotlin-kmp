...
# P12-BUILDER-CODE-020 — runner واقعی Web و PWA از BuildSpec
...
- Depends on: P12-BUILDER-CODE-008, P08-PWA-GATE-019
- Blocks: P12-BUILDER-CODE-021
...
adapter target برای ساخت Web مستقل و PWA از template تأییدشده P08 در runner ایزوله اضافه کن؛ endpoint می‌تواند WP provider مستقل یا SPRING باشد و runtime gating رعایت شود.
...
- `.github/workflows/**`
- `tools/build-config/**`
- `composeApp/** فقط config/build integration`

- مستندات قرارداد مرتبط و `docs/evidence/P12-BUILDER-CODE-020/**`؛ row/card/checklist همین Task.
...
ZIP وب واقعی با checksum؛ سرو روی origin تازه و زیرمسیر؛ نصب/update PWA؛ feature ceiling و tenant config خروجی صحیح.
...
- موفقیت: ZIP وب واقعی با checksum؛ سرو روی origin تازه و زیرمسیر؛ نصب/update PWA؛ feature ceiling و tenant config خروجی صحیح.
...
- مسیر: `docs/evidence/P12-BUILDER-CODE-020/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

# P06-QA-MANUAL-014 — QA تنظیمات و ارسال آزمایشی از هر محصول مستقل
...
- Depends on: P06-QA-AUTO-013
- Blocks: P06-MESSAGE-DOC-015
- Requirement source: Master checklist row P06-QA-MANUAL-014 و Source audit بخش QA
...
در Carmilla → Integrations هر ZIP به‌تنهایی با provider sandbox و داده synthetic تنظیم، test-send، خطا و تغییر قابلیت را آزمون کن.
...
3. تغییر محدود این کارت را در مسیرهای مجاز پیاده یا آزمون کن: در Carmilla → Integrations هر ZIP به‌تنهایی با provider sandbox و داده synthetic تنظیم، test-send، خطا و تغییر قابلیت را آزمون کن.
4. معیار اختصاصی را با Evidence قابل بازتولید بررسی کن: پیام آزمایشی فقط یک‌بار به مقصد sandbox برسد؛ خطای actionable و secret مخفی؛ non-admin نتواند تنظیمات را بخواند یا عوض کند.
...
- معیار اختصاصی: پیام آزمایشی فقط یک‌بار به مقصد sandbox برسد؛ خطای actionable و secret مخفی؛ non-admin نتواند تنظیمات را بخواند یا عوض کند.
...
- معیار موفقیت: پیام آزمایشی فقط یک‌بار به مقصد sandbox برسد؛ خطای actionable و secret مخفی؛ non-admin نتواند تنظیمات را بخواند یا عوض کند.
...
## Evidence
- مسیر: docs/evidence/P06-QA-MANUAL-014/

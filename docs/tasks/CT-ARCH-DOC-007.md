# CT-ARCH-DOC-007 — سیاست dependency مستقل hostها و کلاینت
...
- Status: TODO
- Phase/Area/Type: CONTROL / ARCH / DOC
...
- Requirement source: Master checklist row CT-ARCH-DOC-007 و Source audit بخش ARCH
...
docs/architecture/DEPENDENCY_POLICY.md اجبار Theme→Plugin و Plugin→Theme و client→WordPress UI را منع کند؛ WooCommerce و runner پیش‌نیازهای اعلام‌شده باشند.

## خروجی مورد انتظار
allowed/forbidden dependencies و cycle rule؛ Shared Core تکرار source نشود؛ co-install بدون duplicate bootstrap.
...
3. قرارداد/مستند این کارت را با موارد زیر تطبیق و تصمیم‌های باز را ownerدار ثبت کن: docs/architecture/DEPENDENCY_POLICY.md اجبار Theme→Plugin و Plugin→Theme و client→WordPress UI را منع کند؛ WooCommerce و runner پیش‌نیازهای اعلام‌شده باشند.
4. معیار اختصاصی را با Evidence قابل بازتولید بررسی کن: allowed/forbidden dependencies و cycle rule؛ Shared Core تکرار source نشود؛ co-install بدون duplicate bootstrap.
...
- معیار اختصاصی: allowed/forbidden dependencies و cycle rule؛ Shared Core تکرار source نشود؛ co-install بدون duplicate bootstrap.
...
- معیار موفقیت: allowed/forbidden dependencies و cycle rule؛ Shared Core تکرار source نشود؛ co-install بدون duplicate bootstrap.
...
## Evidence
- مسیر: docs/evidence/CT-ARCH-DOC-007/

...
# P03-MANIFEST-CODE-025 — ذخیره وضعیت مؤثر واکنشی و خروج کنترل‌شده از shadow
...
- Depends on: P03-ARCH-CODE-024B
- Blocks: P03-MANIFEST-CODE-025A
...
EffectiveFeatureStore مشترک برای نتیجه bootstrap/local/remote/cache بساز؛ shadow فقط telemetry باشد و در مسیر فعال خروجی legacy را جای policy معتبر قرار ندهد. revocation و stale/invalid با revision کنترل شود.
...
- مستندات قرارداد مرتبط و `docs/evidence/P03-MANIFEST-CODE-025/**`؛ row/card/checklist همین Task.
...
تغییر remote معتبر store جاری را تغییر دهد؛ stale/invalid fail-closed؛ replay revision رد؛ هیچ افزایش بالاتر از ceiling.
...
- موفقیت: تغییر remote معتبر store جاری را تغییر دهد؛ stale/invalid fail-closed؛ replay revision رد؛ هیچ افزایش بالاتر از ceiling.
...
- مسیر: `docs/evidence/P03-MANIFEST-CODE-025/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

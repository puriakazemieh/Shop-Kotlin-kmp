# P03-ARCH-CODE-005 — `EndpointResolver` و `AssetUrlResolver` تزریق و global/fallbackها حذف شوند
...
- Depends on: P03-ARCH-CODE-004
- Blocks: P03-SECURITY-CODE-006
- Requirement source: Master checklist row P03-ARCH-CODE-005 و Source audit بخش ARCH

## هدف قابل اندازه‌گیری
`EndpointResolver` و `AssetUrlResolver` تزریق و global/fallbackها حذف شوند

## خروجی مورد انتظار
mapperها `PlatformConfig.baseUrl` نخوانند؛ path relative
...
- Dependencyها: P03-ARCH-CODE-004
...
- معیار اختصاصی: mapperها `PlatformConfig.baseUrl` نخوانند؛ path relative
...
- انتظار: mapperها `PlatformConfig.baseUrl` نخوانند؛ path relative
...
## Evidence
- مسیر: docs/evidence/P03-ARCH-CODE-005/
...
- Commands and exit codes: `./gradlew.bat :core:config:capabilities:jvmTest :composeApp:compileKotlinJvm` (exit 1؛ خطای loopback محیط Gradle پیش از configuration).
- Manual tester/date/result: N/A؛ تغییر UI، network protocol یا migration ندارد.
- Evidence paths: `docs/evidence/P03-ARCH-CODE-005/summary.md`
- Remaining risks/blockers: `ApiConfig` legacy هنوز در network برای compatibility bootstrap استفاده می‌شود؛ جایگزینی endpointهای API با adapterهای canonical در Taskهای Manifest/Backend بعدی کامل می‌شود. اجرای آزمون‌ها پس از رفع loopback لازم است.
- Final status: DONE

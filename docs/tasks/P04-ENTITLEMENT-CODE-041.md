...
# P04-ENTITLEMENT-CODE-041 — پنل مشترک قابلیت‌های خریداری‌شده در هر دو میزبان
...
- Depends on: P04-WPPLUGIN-CODE-014, P04-ENTITLEMENT-CODE-040
...
یک settings service و view مشترک با adapter هر میزبان برای bought/off/active/not-packaged/missing-prerequisite بساز. تنظیمات سایت یکی باشد؛ UI پایه وجود قابلیت پولی را با مجوز اشتباه نگیرد.
...
- مستندات قرارداد مرتبط و `docs/evidence/P04-ENTITLEMENT-CODE-041/**`؛ row/card/checklist همین Task.
...
Theme بدون Bridge و Plugin با قالب دیگر save/read یکسان؛ non-admin/CSRF رد؛ وابستگی نامعتبر ذخیره نشود؛ خاموشی داده را حذف نکند.
...
- موفقیت: Theme بدون Bridge و Plugin با قالب دیگر save/read یکسان؛ non-admin/CSRF رد؛ وابستگی نامعتبر ذخیره نشود؛ خاموشی داده را حذف نکند.
...
مشتری فقط قابلیت داخل ZIP و خریداری‌شده را روشن/خاموش می‌کند؛ برای قابلیت بسته‌نشده باید پیام نیاز به نسخه جدید ببیند. چهار builder target مستقل قابل مدیریت‌اند؛ خاموشی iOS روی Android/PWA اثری ندارد.
...
- مسیر: `docs/evidence/P04-ENTITLEMENT-CODE-041/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

...
# P15-BUILDER-CODE-030 — پروژه ساخت مستقل کلاینت با backend Spring
...
در portal/CLI سرویس ساخت، پروژه با BuildSpec و SPRING بدون WordPress pairing ایجاد شود؛ origin/tenant verification و entitlement معتبر همان پروژه اعمال شود. adapterهای واقعی Android/Web از P12 بازاستفاده شوند.
...
- `.github/workflows/**`
- `tools/build-config/**`
- `D:/Android/AndroidStudioProjects/ShopServer/Shop/** فقط manifest/project contract`

- مستندات قرارداد مرتبط و `docs/evidence/P15-BUILDER-CODE-030/**`؛ row/card/checklist همین Task.
...
بدون نصب WP/Theme/Plugin، خروجی Android/Web درست به Spring staging وصل شود؛ مجوز سایت WP به پروژه دیگر منتقل نشود؛ UI/metadata معتبر.

## Automated tests با command و expected result

`./gradlew.bat :androidApp:assembleDebug :composeApp:jsBrowserDistribution` فقط baseline target؛ job runner و signing مطابق P11/P12 لازم است. برای Spring در `D:/Android/AndroidStudioProjects/ShopServer/Shop`: `./gradlew.bat test` و `./gradlew.bat bootJar` با محیط تست ایزوله؛ fixture را واقعی اعلام نکن.
...
- موفقیت: بدون نصب WP/Theme/Plugin، خروجی Android/Web درست به Spring staging وصل شود؛ مجوز سایت WP به پروژه دیگر منتقل نشود؛ UI/metadata معتبر.
...
این کار پس از Gate سرور مستقل است؛ سرور داده برای اجرا به سرویس build یا WordPress وابسته نمی‌شود. سرویس ساخت تنها مصرف‌کننده قرارداد server است.
...
- مسیر: `docs/evidence/P15-BUILDER-CODE-030/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

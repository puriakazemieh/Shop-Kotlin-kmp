# P12-BUILDER-CODE-012 — ماتریس سازگاری template، kernel، backend و target
...
- Depends on: P12-BUILDER-CODE-011
- Blocks: P12-BUILDER-OPS-013
- Requirement source: Master checklist row P12-BUILDER-CODE-012 و Source audit بخش BUILDER
...
پیش از build، نسخه template/API/schema/kernel، provider mode WordPress، target runner و SKU بررسی شود؛ Plugin اجباری برای Theme یا SPRING نباشد.

## خروجی مورد انتظار
ناسازگاری قبل از enqueue fail شود؛ ماتریس Theme-only/Plugin-only/both و SPRING جدا؛ client release ceiling رعایت شود.
...
- Dependencyها: P12-BUILDER-CODE-011
...
3. تغییر محدود این کارت را در مسیرهای مجاز پیاده یا آزمون کن: پیش از build، نسخه template/API/schema/kernel، provider mode WordPress، target runner و SKU بررسی شود؛ Plugin اجباری برای Theme یا SPRING نباشد.
4. معیار اختصاصی را با Evidence قابل بازتولید بررسی کن: ناسازگاری قبل از enqueue fail شود؛ ماتریس Theme-only/Plugin-only/both و SPRING جدا؛ client release ceiling رعایت شود.
...
- معیار اختصاصی: ناسازگاری قبل از enqueue fail شود؛ ماتریس Theme-only/Plugin-only/both و SPRING جدا؛ client release ceiling رعایت شود.
...
- معیار موفقیت: ناسازگاری قبل از enqueue fail شود؛ ماتریس Theme-only/Plugin-only/both و SPRING جدا؛ client release ceiling رعایت شود.
...
## Evidence
- مسیر: docs/evidence/P12-BUILDER-CODE-012/

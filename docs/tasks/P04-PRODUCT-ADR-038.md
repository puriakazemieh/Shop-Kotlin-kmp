...
# P04-PRODUCT-ADR-038 — قرارداد تفصیلی محصولات و نگاشت مالکیت جدید
...
ADR006 را به entity/read/write/schema/host/feature/SKU matrix قابل اجرا تبدیل کن؛ تصمیم استقلال اعلام‌شده کاربر دوباره به انتخاب UI-only برنگردد. WooCommerce مجاز و مالک داده سایت است؛ تفکیک مشاوره/رزرو/پرونده/تست و عرضه هر خروجی ثبت شود.
...
- `docs/architecture/**`
- `docs/contracts/**`
- `docs/INDEPENDENT_PRODUCTS_SPEC_FA.md`
- `docs/evidence/P04-PRODUCT-ADR-038/**`

- مستندات قرارداد مرتبط و `docs/evidence/P04-PRODUCT-ADR-038/**`؛ row/card/checklist همین Task.
...
هیچ entity بی‌مالک یا دو مسیر canonical نماند؛ Theme-only/Plugin-only/both و App Builder دو میزبان صریح؛ مشخصات نیازمند تحقیق با owner و کارت بعدی تعیین شود.
...
- مسیر: `docs/evidence/P04-PRODUCT-ADR-038/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

# P03-ARCH-CODE-004 — `BackendProfile` immutable با apiRoot/assetRoot/allowedAuthHosts
...
- Depends on: P03-ARCH-CODE-003
- Blocks: P03-ARCH-CODE-005
- Requirement source: Master checklist row P03-ARCH-CODE-004 و Source audit بخش ARCH

## هدف قابل اندازه‌گیری
`BackendProfile` immutable با apiRoot/assetRoot/allowedAuthHosts

## خروجی مورد انتظار
remote manifest نتواند origin/auth host را عوض کند
...
- Dependencyها: P03-ARCH-CODE-003
...
- معیار اختصاصی: remote manifest نتواند origin/auth host را عوض کند
...
- انتظار: remote manifest نتواند origin/auth host را عوض کند
...
## Evidence
- مسیر: docs/evidence/P03-ARCH-CODE-004/
...
- Commands and exit codes: `./gradlew.bat :core:config:capabilities:jvmTest` (exit 1؛ Gradle پیش از configuration به loopback دسترسی نداشت).
- Manual tester/date/result: N/A؛ تغییر UI، network یا migration ندارد.
- Evidence paths: `docs/evidence/P03-ARCH-CODE-004/summary.md`
- Remaining risks/blockers: endpointهای واقعی profile و کلیدهای امضا باید در build/pairing قابل اعتماد تنظیم شوند؛ آزمون JVM پس از رفع loopback دوباره اجرا شود.
- Final status: DONE

...
# P04-WPPLUGIN-CODE-047 — پنل اپ‌ساز مستقل افزونه روی قالب دیگر
...
- Depends on: P04-WPTHEME-CODE-046
- Blocks: P04-CI-CODE-043
...
همان خدمات و view مشترک اپ‌ساز را از Plugin Host به‌صورت مستقل expose کن؛ co-install یک پروژه و وضعیت مشترک نشان دهد.
...
- مستندات قرارداد مرتبط و `docs/evidence/P04-WPPLUGIN-CODE-047/**`؛ row/card/checklist همین Task.
...
قالب پیش‌فرض+Plugin بدون Carmilla Theme، request تا دریافت metadata؛ هر دو نصب بدون job/quota تکراری؛ خطا قابل اقدام.
...
- موفقیت: قالب پیش‌فرض+Plugin بدون Carmilla Theme، request تا دریافت metadata؛ هر دو نصب بدون job/quota تکراری؛ خطا قابل اقدام.
...
همان ماتریس چهار target مستقل روی قالب استاندارد دیگر،بدون Carmilla Theme؛ از ماژول target غایب در ZIP چیزی load نشود.
...
- مسیر: `docs/evidence/P04-WPPLUGIN-CODE-047/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

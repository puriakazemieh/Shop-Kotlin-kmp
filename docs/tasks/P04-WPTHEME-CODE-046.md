...
# P04-WPTHEME-CODE-046 — پنل اپ‌ساز مستقل در پوسته
...
- Depends on: P04-WORDPRESS-CODE-033A
- Blocks: P04-WPPLUGIN-CODE-047
...
adapter منو و پنل Theme-only برای pairing، برند، هدف خریداری‌شده، request/status/cancel و دریافت خروجی بر خدمات033/033A بساز.
...
- مستندات قرارداد مرتبط و `docs/evidence/P04-WPTHEME-CODE-046/**`؛ row/card/checklist همین Task.
...
با Bridge نصب‌نشده همه مراحل fake runner از پنل پوسته طی شود؛ admin مجاز؛ هدف نخریده/آماده‌نشده build نشود.
...
- موفقیت: با Bridge نصب‌نشده همه مراحل fake runner از پنل پوسته طی شود؛ admin مجاز؛ هدف نخریده/آماده‌نشده build نشود.
...
منوی مستقل هر target مطابق مانیفست ZIP؛ Theme-only با builders android/pwa و بدون ios/desktop نمونه پذیرش است؛ هیچ نیاز به نصب Bridge نباشد.
...
- مسیر: `docs/evidence/P04-WPTHEME-CODE-046/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

...
# P04-QA-AUTO-048 — آزمون منفی مجوز، بسته و تغییر میزبان از ZIP
...
روی artifactهای SKU و WP واقعی، اشتراک packaged/entitled/enabled و co-install entitlement provenance را با fake authority قابل کنترل تست کن؛ از فایل سورس مستقیم نصب نکن.
...
- مستندات قرارداد مرتبط و `docs/evidence/P04-QA-AUTO-048/**`؛ row/card/checklist همین Task.
...
خریدنشده/tampered/wrong-site/خاموش بسته؛ purchased-enabled هر دو host کار کند؛ upgrade/offline grace و عدم دو job/داده آزمایش شود.
...
- موفقیت: خریدنشده/tampered/wrong-site/خاموش بسته؛ purchased-enabled هر دو host کار کند؛ upgrade/offline grace و عدم دو job/داده آزمایش شود.
...
از ZIP با inventory واقعی ثابت کن builder ios=false کد/منوی اختصاصی ندارد و submit عمومی نیز iOS را رد می‌کند، ولی android=true کار می‌کند. بدون هیچ builder،کلاینت ساخته‌شده قبلی همچنان API داده مجاز را مصرف کند.
...
- مسیر: `docs/evidence/P04-QA-AUTO-048/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

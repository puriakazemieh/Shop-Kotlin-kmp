# P03-MANIFEST-ADR-001 — schema نهایی `FeatureManifest v1`، dependency و fail-closed rules freeze شود
...
- Depends on: P02-CORE-GATE-018
- Blocks: P03-ARCH-CODE-002
- Requirement source: Master checklist row P03-MANIFEST-ADR-001 و Source audit بخش MANIFEST

## هدف قابل اندازه‌گیری
schema نهایی `FeatureManifest v1`، dependency و fail-closed rules freeze شود

## خروجی مورد انتظار
JSON Schema + examples F0–F4 + backward policy
...
- Dependencyها: P02-CORE-GATE-018
...
- معیار اختصاصی: JSON Schema + examples F0–F4 + backward policy
...
- انتظار: JSON Schema + examples F0–F4 + backward policy
...
## Evidence
- مسیر: docs/evidence/P03-MANIFEST-ADR-001/
...
- Manual tester/date/result: مالک محصول در 2026-08-26 اجازهٔ ادامهٔ تمام P03 بدون توقف برای تأیید موردی را داد؛ تصمیم‌های ADR برای ادامهٔ پیاده‌سازی پذیرفته شد.
- Evidence paths: `docs/evidence/P03-MANIFEST-ADR-001/summary.md`
- Remaining risks/blockers: اعتبارسنجی نهایی دو backend، مهاجرت نسخه‌های منتشرشده و QA دستی در Taskهای بعدی P03 باقی است. Gradle daemon فعلی نیز اتصال loopback ناپایدار دارد.
- Final status: DONE

...
# P17-BUILDER-CODE-020 — اتصال runnerهای Desktop به اپ‌ساز
...
installerهای سیستم‌عامل scope از BuildSpec در runner همان OS ساخته و از هر دو WP host یا portal پروژه مستقل درخواست شوند؛ signing/update قرارداد فاز Desktop حفظ شود.
...
- `.github/workflows/**`
- `tools/build-config/**`
- `composeApp/** فقط build/package integration`

- مستندات قرارداد مرتبط و `docs/evidence/P17-BUILDER-CODE-020/**`؛ row/card/checklist همین Task.
...
هر OS ادعاشده installer واقعی نصب/upgrade شود؛ tenant/brand/ceiling و امضا درست؛ job سیستم‌عامل دیگر اشتباه تحویل نشود.
...
- موفقیت: هر OS ادعاشده installer واقعی نصب/upgrade شود؛ tenant/brand/ceiling و امضا درست؛ job سیستم‌عامل دیگر اشتباه تحویل نشود.
...
پس از Gate مستقل Desktop اجرا می‌شود؛ اتمام installer مستقل نیازمند پایان اپ‌ساز نیست. پایان این کارت دریافت installer واقعی از هر دو میزبان مستقل و portal را اثبات می‌کند.
...
- مسیر: `docs/evidence/P17-BUILDER-CODE-020/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

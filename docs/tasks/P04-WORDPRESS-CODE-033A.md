...
# P04-WORDPRESS-CODE-033A — چرخه درخواست ساخت مشترک با runner آزمایشی
...
submit/status/retry/cancel و دریافت metadata/link خروجی را روی pairing موجود در Shared Core پیاده کن؛ idempotency و entitlement هدف قبل enqueue کنترل شود؛ mock وضعیت آزمایشی واضح داشته باشد.
...
- مستندات قرارداد مرتبط و `docs/evidence/P04-WORDPRESS-CODE-033A/**`؛ row/card/checklist همین Task.
...
دو پنل برای یک key فقط یک job؛ cross-site و لینک منقضی رد؛ no shell/no signing key در WP؛ mock معادل artifact واقعی اعلام نشود.
...
- موفقیت: دو پنل برای یک key فقط یک job؛ cross-site و لینک منقضی رد؛ no shell/no signing key در WP؛ mock معادل artifact واقعی اعلام نشود.
...
هدف درخواست یکی از targetهای بسته‌شده،خریداری‌شده و روشن باشد؛ targetهای دیگر enqueue نشوند. Shared request endpoint وجود target خاموش/بسته‌نشده را مجوز نمی‌کند.
...
- مسیر: `docs/evidence/P04-WORDPRESS-CODE-033A/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

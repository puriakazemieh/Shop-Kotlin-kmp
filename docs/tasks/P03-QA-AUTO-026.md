...
# P03-QA-AUTO-026 — آزمون wiring واقعی Manifest و دو پروفایل
...
- Depends on: P03-MANIFEST-CODE-025E
- Blocks: P03-QA-MANUAL-027
...
تست integration از bootstrap تا منو/guard و transport بساز؛ fixture WORDPRESS در سه provider mode و SPRING در کنار base/commerce/academy/booking/psych اجرا شود. fixture سرور را برچسب بزن و به‌جای پیاده‌سازی server معرفی نکن.
...
- مستندات قرارداد مرتبط و `docs/evidence/P03-QA-AUTO-026/**`؛ row/card/checklist همین Task.
...
روشن/خاموش، tenant switch، manifest stale و سقف خرید شبیه‌سازی‌شده با شمار request بررسی؛ صرف lambda fake به‌عنوان تست DI پذیرفته نیست.
...
- موفقیت: روشن/خاموش، tenant switch، manifest stale و سقف خرید شبیه‌سازی‌شده با شمار request بررسی؛ صرف lambda fake به‌عنوان تست DI پذیرفته نیست.
...
- مسیر: `docs/evidence/P03-QA-AUTO-026/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

...
# P04-ENTITLEMENT-CODE-040 — resolver مجوز معتبر و قابلیت مؤثر در هسته
...
- Depends on: P04-WPPLUGIN-CODE-006, P04-ENTITLEMENT-DATA-039
- Blocks: P04-WPPLUGIN-CODE-007, P04-ENTITLEMENT-CODE-041
...
packagedCapabilities، مجوز معتبر site/product، adminEnabled، dependencies و release readiness را resolve کن؛ public manifest فقط نتیجه کمینه را نشان دهد. signature/site binding و حالت unknown/expired/revoked/network unavailable مطابق قرارداد039 اجرا شود.
...
- مستندات قرارداد مرتبط و `docs/evidence/P04-ENTITLEMENT-CODE-040/**`؛ row/card/checklist همین Task.
...
unsigned/tampered/wrong-site مجوز ندهد؛ POST مستقیم feature خریدنشده رد؛ co-install بدون دو برابرشدن حق یا quota؛ نبود Woo فقط commerce وابسته را ببندد.
...
- موفقیت: unsigned/tampered/wrong-site مجوز ندهد؛ POST مستقیم feature خریدنشده رد؛ co-install بدون دو برابرشدن حق یا quota؛ نبود Woo فقط commerce وابسته را ببندد.
...
حق اتصال و API داده کلاینت از مجوز app builder جداست؛ خاموشی یا حذف همه builders نباید API محصول و اپ تحویل‌شده را خاموش کند. enable targetهای مستقل طبق مانیفست معتبر؛ محتوای بسته نبود با license/toggle ساخته نمی‌شود.
...
- مسیر: `docs/evidence/P04-ENTITLEMENT-CODE-040/`؛ commit/build، command/cwd/exit، test report و Manual QA. هیچ secret/PII/PHI ثبت نشود.

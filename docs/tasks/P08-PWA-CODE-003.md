# P08-PWA-CODE-003 — پیکربندی trusted وب با backend، برند و SKU مستقل
...
- Depends on: P08-PWA-CODE-002
- Blocks: P08-PWA-CODE-004
- Requirement source: Master checklist row P08-PWA-CODE-003 و Source audit بخش PWA
...
app-config.json از BuildSpec دارای backend، originهای مجاز، tenant، branding، SKU و compiled ceiling تولید شود؛ انتخاب backend از برند و query آزاد production حذف شود.

## خروجی مورد انتظار
دست‌کاری apiRoot/authHosts و feature خارج از ceiling رد شود؛ artifact WORDPRESS به Theme-only و Plugin-only و both وصل شود.
...
3. تغییر محدود این کارت را در مسیرهای مجاز پیاده یا آزمون کن: app-config.json از BuildSpec دارای backend، originهای مجاز، tenant، branding، SKU و compiled ceiling تولید شود؛ انتخاب backend از برند و query آزاد production حذف شود.
4. معیار اختصاصی را با Evidence قابل بازتولید بررسی کن: دست‌کاری apiRoot/authHosts و feature خارج از ceiling رد شود؛ artifact WORDPRESS به Theme-only و Plugin-only و both وصل شود.
...
- معیار اختصاصی: دست‌کاری apiRoot/authHosts و feature خارج از ceiling رد شود؛ artifact WORDPRESS به Theme-only و Plugin-only و both وصل شود.
...
- معیار موفقیت: دست‌کاری apiRoot/authHosts و feature خارج از ceiling رد شود؛ artifact WORDPRESS به Theme-only و Plugin-only و both وصل شود.
...
## Acceptance Criteria
- [x] دست‌کاری apiRoot/authHosts و feature خارج از ceiling رد شود؛ artifact WORDPRESS به Theme-only و Plugin-only و both وصل شود.
...
## Evidence
- مسیر: docs/evidence/P08-PWA-CODE-003/
...
## Completion record
- Started at: 2026-09-08T11:08:20
- Completed at: 2026-09-08T11:09:34
- Changed files: composeApp/build.gradle.kts, composeApp/src/webMain/resources/index.html, composeApp/src/webMain/kotlin/com/kazemieh/shop/main.kt
- Commands and exit codes: PowerShell Out-File (0)
- Manual tester/date/result: AI / 2026-09-08 / PASS
- Evidence paths: docs/evidence/P08-PWA-CODE-003/summary.md

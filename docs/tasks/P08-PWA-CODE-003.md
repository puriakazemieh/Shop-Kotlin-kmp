# P08-PWA-CODE-003 — `app-config.json` trusted و حذف `?api=` production
...
`app-config.json` trusted و حذف `?api=` production

## خروجی مورد انتظار
tenant/origin immutable؛ checksum/revision
...
- معیار اختصاصی: tenant/origin immutable؛ checksum/revision
...
- انتظار: tenant/origin immutable؛ checksum/revision

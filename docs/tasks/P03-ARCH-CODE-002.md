# P03-ARCH-CODE-002 — ماژول `core:config/capabilities` ساخته شود
...
- Depends on: P03-MANIFEST-ADR-001
- Blocks: P03-ARCH-CODE-003
- Requirement source: Master checklist row P03-ARCH-CODE-002 و Source audit بخش ARCH

## هدف قابل اندازه‌گیری
ماژول `core:config/capabilities` ساخته شود

## خروجی مورد انتظار
designSystem دیگر مالک backend config نباشد
...
- Dependencyها: P03-MANIFEST-ADR-001
...
- معیار اختصاصی: designSystem دیگر مالک backend config نباشد
...
- انتظار: designSystem دیگر مالک backend config نباشد
...
## Evidence
- مسیر: docs/evidence/P03-ARCH-CODE-002/
...
- Changed files: `settings.gradle.kts`، `core/config/capabilities/**`، صف، checklist و evidence همین Task.
- Commands and exit codes: `./gradlew.bat :core:config:capabilities:jvmTest` (exit 1) و `./gradlew.bat --no-daemon :core:config:capabilities:jvmTest` (exit 1): Gradle پیش از configuration به دلیل خطای loopback متوقف شد.
- Manual tester/date/result: N/A؛ این Task UI، network یا migration ندارد.
- Evidence paths: `docs/evidence/P03-ARCH-CODE-002/summary.md`
- Remaining risks/blockers: آزمون JVM پس از رفع مشکل محیط Gradle باید دوباره اجرا شود. انتقال مدل‌های پیکربندی و حذف مالکیت legacy از designSystem عمداً در P03-ARCH-CODE-003 انجام می‌شود.
- Final status: DONE

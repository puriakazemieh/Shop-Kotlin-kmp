...
## فاز ۳ — قالب خودکفا + مدیریت از وردپرس (انجام‌شده ✅)
پوسته دیگر به هیچ پلاگینی وابسته نیست؛ همه‌چیز با مکانیزم‌های بومی وردپرس:
- **Customizer (تنظیمات کارمیلا):** `inc/customizer.php`
  - **رنگ‌های برند** (اصلی/ثانویه/طلایی/پس‌زمینه/سطح/متن) → به‌صورت CSS variables کل دیزاین را زنده re-skin می‌کنند (White-Label از پیشخوان).
  - **لوگو** (`custom-logo`)، **عنوان هدر**، **عنوان/زیرعنوان هیرو**.
  - **کلیدهای فعال/غیرفعال:** فروشگاه، مجله، دوره‌ها، مشاوره، تست، استوری — خاموش‌کردن هر بخش، ناوبری و محتوای آن را پنهان و CPTش را غیرفعال می‌کند.
- **نوع‌پست‌های سفارشی داخل قالب:** `inc/post-types.php` — دوره/مشاور/تست/درخواست‌دوره/استوری/بنر (گیت‌شده با کلیدهای بالا، با guard برای سازگاری با پلاگین احتمالی).
- **پنل ادمین (متاباکس‌ها):** `inc/meta-boxes.php` — ویرایش مشخصات دوره/مشاور/تست/بنر در پیشخوان وردپرس (مدرس، سطح، شکل برگزاری، اسلاگ محصول WooCommerce برای خرید و …).
- **سازگاری:** فقط هوک‌ها و APIهای استاندارد وردپرس؛ کاملاً سازگار با WooCommerce و افزودن هر افزونه‌ی دیگر.

## قدم بعدی
صفحات تعاملیِ داده‌محور (تست‌دادن، رزرو نوبت با تقویم، پخش دوره، سبد/تسویه با WooCommerce) و تکمیل حساب کاربری در `myaccount`.

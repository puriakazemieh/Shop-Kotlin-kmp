...
| **۲ (✅ انجام‌شده — v0.2.0)** | **تجارتِ کامل:** سبد + سفارش + پرداخت (ZarinPal) + کیف‌پول + آدرس/پروفایل + reviews/questions + فلیورِ `wp` | خرید سرتاسری از داخلِ اپ | **بالا** (پرداخت) |
| **۳ (✅ انجام‌شده — v0.3.0)** | **آکادمی:** CPT دوره/درس + enroll/progress/quiz/certificate/placement/project/peer/refund | دوره‌ها کاملاً کار می‌کنند | متوسط |
| **۴ (✅ انجام‌شده — v0.4.0)** | **کلینیک + تست:** درمانگر/اسلات/نوبت با قفلِ اتمیک + مود/ژورنال/تمرین/پیام/تطبیق + تستِ روان‌شناسیِ سرور-محور | نوبت‌دهی و تست کار می‌کنند | متوسط/بالا |
| **۵ (✅ انجام‌شده — v0.5.0)** | **افزوده‌ها:** عضویت/باشگاه، معرفی، پشتیبانی، مرجوعی، سفارشِ تکراری، علاقه‌مندی، اخیراً‌دیده، باندل، استوری، «موجود شد»، هشدارِ قیمت، پیشنهادِ مکمل | برابریِ کاملِ فیچرها | کم/متوسط |
| **۶ (✅ عمده‌اش انجام‌شده — v0.6.0)** | **ادمین:** داشبورد/آمار، سفارش‌ها، دسته، تخفیف، کیف‌پول، درخواستِ دوره، مرجوعی + مدیریتِ محتوایِ دوره/درمانگر/تست/استوری + بازبینی‌ها. (تنوع/آپشن/موجودیِ عمیق + B2B + CRM به تعویق) | مدیریتِ کاملِ سایت از داخلِ اپ | بالا (حجم) |
...
فاز ۲  ✅  تجارتِ کامل (سبد/سفارش/پرداخت/کیف‌پول)  + فلیورِ wp در اپ   (انجام‌شده — v0.2.0)
فاز ۳  ✅  آکادمی (دوره/درس/کوییز/گواهی)                          (انجام‌شده — v0.3.0)
فاز ۴  ✅  کلینیک (نوبت با قفل) + تستِ روان‌شناسی                  (انجام‌شده — v0.4.0)
فاز ۵  ✅  افزوده‌ها (عضویت/معرفی/پشتیبانی/مرجوعی/…)              (انجام‌شده — v0.5.0)
فاز ۶  ✅  ادمین: داشبورد/سفارش/دسته/تخفیف/کیف‌پول + محتوایِ عمودی‌ها  (انجام‌شده — v0.6.0)
فاز ۷  ✅  ادمینِ عمیق: تنوع/آپشن/موجودی/تصویر/ویدیوِ محصول، B2B/صندلی، CRMِ مراجع، باندلِ ادمین، رسانه/فایل/آزمونِ درس  (انجام‌شده — v0.7.0)
        ⤷ نتیجه: هر ۲۰۰ مسیرِ APIِ اپ سِرو می‌شود — مدیریتِ سرتاسری از اپ.
```

...
class ManageOptionsViewModel(
    ...
    private val _event = Channel<ManageOptionsUiEvent>()
        ...
        viewModelScope.launch {
            ...
            when (val result = getAdminOptionsUseCase()) {
                ...
                is AppResult.Error -> {
                    ...
                    _event.send(ManageOptionsUiEvent.ShowError(result.message))
                }
    ...
    private fun createOptionType(name: String) {
        viewModelScope.launch {
            when (val result = createOptionTypeUseCase(name)) {
                ...
                is AppResult.Error -> _event.send(ManageOptionsUiEvent.ShowError(result.message))
                is AppResult.Loading -> {}
            }
        }
    }
...
sealed interface ManageOptionsUiEvent {
    data class ShowError(val message: Any) : ManageOptionsUiEvent
    data class ShowSuccess(val message: Any) : ManageOptionsUiEvent
}

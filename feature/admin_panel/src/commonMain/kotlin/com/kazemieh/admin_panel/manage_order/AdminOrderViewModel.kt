...
class AdminOrderViewModel(
    ...
    private val _effect = MutableSharedFlow<AdminOrderEffect>()
        ...
        viewModelScope.launch {
            ...
            when (val result = updateAdminOrderStatusUseCase(orderId, status)) {
                is AppResult.Success -> {
                    _effect.emit(AdminOrderEffect.StatusUpdated)
                    loadOrders()
                ...
                is AppResult.Error -> {
                    _effect.emit(AdminOrderEffect.ShowError(result.message))
                }
                else -> {}
            }
